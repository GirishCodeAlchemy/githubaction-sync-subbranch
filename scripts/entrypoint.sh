...
for branch in $(git branch -r | grep -v '\->'); do
  if [ "$branch" != "origin/main" ] && [ "$branch" != "origin/master" ]; then
    ...
    if [ -n "$conflict" ]; then
      ...
      # Send email notification
      echo "There was a merge conflict when syncing the branch $conflict with main." | mail -s "$subject" -v  $branch_owner
    else
      echo "Merge successful in $branch_name"
      subject="Merge Successful in $branch_name"
      echo "The branch $branch_name was successfully synced with main." | mail -s "$subject" -v  $branch_owner
    fi
  fi
done
